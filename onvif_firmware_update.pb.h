// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: onvif_firmware_update.proto

#ifndef GOOGLE_PROTOBUF_INCLUDED_onvif_5ffirmware_5fupdate_2eproto
#define GOOGLE_PROTOBUF_INCLUDED_onvif_5ffirmware_5fupdate_2eproto

#include <limits>
#include <string>

#include <google/protobuf/port_def.inc>
#if PROTOBUF_VERSION < 3017000
#error This file was generated by a newer version of protoc which is
#error incompatible with your Protocol Buffer headers. Please update
#error your headers.
#endif
#if 3017003 < PROTOBUF_MIN_PROTOC_VERSION
#error This file was generated by an older version of protoc which is
#error incompatible with your Protocol Buffer headers. Please
#error regenerate this file with a newer version of protoc.
#endif

#include <google/protobuf/port_undef.inc>
#include <google/protobuf/io/coded_stream.h>
#include <google/protobuf/arena.h>
#include <google/protobuf/arenastring.h>
#include <google/protobuf/generated_message_table_driven.h>
#include <google/protobuf/generated_message_util.h>
#include <google/protobuf/metadata_lite.h>
#include <google/protobuf/generated_message_reflection.h>
#include <google/protobuf/message.h>
#include <google/protobuf/repeated_field.h>  // IWYU pragma: export
#include <google/protobuf/extension_set.h>  // IWYU pragma: export
#include <google/protobuf/unknown_field_set.h>
// @@protoc_insertion_point(includes)
#include <google/protobuf/port_def.inc>
#define PROTOBUF_INTERNAL_EXPORT_onvif_5ffirmware_5fupdate_2eproto
PROTOBUF_NAMESPACE_OPEN
namespace internal {
class AnyMetadata;
}  // namespace internal
PROTOBUF_NAMESPACE_CLOSE

// Internal implementation detail -- do not use these members.
struct TableStruct_onvif_5ffirmware_5fupdate_2eproto {
  static const ::PROTOBUF_NAMESPACE_ID::internal::ParseTableField entries[]
    PROTOBUF_SECTION_VARIABLE(protodesc_cold);
  static const ::PROTOBUF_NAMESPACE_ID::internal::AuxiliaryParseTableField aux[]
    PROTOBUF_SECTION_VARIABLE(protodesc_cold);
  static const ::PROTOBUF_NAMESPACE_ID::internal::ParseTable schema[2]
    PROTOBUF_SECTION_VARIABLE(protodesc_cold);
  static const ::PROTOBUF_NAMESPACE_ID::internal::FieldMetadata field_metadata[];
  static const ::PROTOBUF_NAMESPACE_ID::internal::SerializationTable serialization_table[];
  static const ::PROTOBUF_NAMESPACE_ID::uint32 offsets[];
};
extern const ::PROTOBUF_NAMESPACE_ID::internal::DescriptorTable descriptor_table_onvif_5ffirmware_5fupdate_2eproto;
namespace onvif_firmware_update {
class UpdateFirmwareReply;
struct UpdateFirmwareReplyDefaultTypeInternal;
extern UpdateFirmwareReplyDefaultTypeInternal _UpdateFirmwareReply_default_instance_;
class UpdateFirmwareRequest;
struct UpdateFirmwareRequestDefaultTypeInternal;
extern UpdateFirmwareRequestDefaultTypeInternal _UpdateFirmwareRequest_default_instance_;
}  // namespace onvif_firmware_update
PROTOBUF_NAMESPACE_OPEN
template<> ::onvif_firmware_update::UpdateFirmwareReply* Arena::CreateMaybeMessage<::onvif_firmware_update::UpdateFirmwareReply>(Arena*);
template<> ::onvif_firmware_update::UpdateFirmwareRequest* Arena::CreateMaybeMessage<::onvif_firmware_update::UpdateFirmwareRequest>(Arena*);
PROTOBUF_NAMESPACE_CLOSE
namespace onvif_firmware_update {

// ===================================================================

class UpdateFirmwareRequest final :
    public ::PROTOBUF_NAMESPACE_ID::Message /* @@protoc_insertion_point(class_definition:onvif_firmware_update.UpdateFirmwareRequest) */ {
 public:
  inline UpdateFirmwareRequest() : UpdateFirmwareRequest(nullptr) {}
  ~UpdateFirmwareRequest() override;
  explicit constexpr UpdateFirmwareRequest(::PROTOBUF_NAMESPACE_ID::internal::ConstantInitialized);

  UpdateFirmwareRequest(const UpdateFirmwareRequest& from);
  UpdateFirmwareRequest(UpdateFirmwareRequest&& from) noexcept
    : UpdateFirmwareRequest() {
    *this = ::std::move(from);
  }

  inline UpdateFirmwareRequest& operator=(const UpdateFirmwareRequest& from) {
    CopyFrom(from);
    return *this;
  }
  inline UpdateFirmwareRequest& operator=(UpdateFirmwareRequest&& from) noexcept {
    if (this == &from) return *this;
    if (GetOwningArena() == from.GetOwningArena()) {
      InternalSwap(&from);
    } else {
      CopyFrom(from);
    }
    return *this;
  }

  static const ::PROTOBUF_NAMESPACE_ID::Descriptor* descriptor() {
    return GetDescriptor();
  }
  static const ::PROTOBUF_NAMESPACE_ID::Descriptor* GetDescriptor() {
    return default_instance().GetMetadata().descriptor;
  }
  static const ::PROTOBUF_NAMESPACE_ID::Reflection* GetReflection() {
    return default_instance().GetMetadata().reflection;
  }
  static const UpdateFirmwareRequest& default_instance() {
    return *internal_default_instance();
  }
  static inline const UpdateFirmwareRequest* internal_default_instance() {
    return reinterpret_cast<const UpdateFirmwareRequest*>(
               &_UpdateFirmwareRequest_default_instance_);
  }
  static constexpr int kIndexInFileMessages =
    0;

  friend void swap(UpdateFirmwareRequest& a, UpdateFirmwareRequest& b) {
    a.Swap(&b);
  }
  inline void Swap(UpdateFirmwareRequest* other) {
    if (other == this) return;
    if (GetOwningArena() == other->GetOwningArena()) {
      InternalSwap(other);
    } else {
      ::PROTOBUF_NAMESPACE_ID::internal::GenericSwap(this, other);
    }
  }
  void UnsafeArenaSwap(UpdateFirmwareRequest* other) {
    if (other == this) return;
    GOOGLE_DCHECK(GetOwningArena() == other->GetOwningArena());
    InternalSwap(other);
  }

  // implements Message ----------------------------------------------

  inline UpdateFirmwareRequest* New() const final {
    return new UpdateFirmwareRequest();
  }

  UpdateFirmwareRequest* New(::PROTOBUF_NAMESPACE_ID::Arena* arena) const final {
    return CreateMaybeMessage<UpdateFirmwareRequest>(arena);
  }
  using ::PROTOBUF_NAMESPACE_ID::Message::CopyFrom;
  void CopyFrom(const UpdateFirmwareRequest& from);
  using ::PROTOBUF_NAMESPACE_ID::Message::MergeFrom;
  void MergeFrom(const UpdateFirmwareRequest& from);
  private:
  static void MergeImpl(::PROTOBUF_NAMESPACE_ID::Message*to, const ::PROTOBUF_NAMESPACE_ID::Message&from);
  public:
  PROTOBUF_ATTRIBUTE_REINITIALIZES void Clear() final;
  bool IsInitialized() const final;

  size_t ByteSizeLong() const final;
  const char* _InternalParse(const char* ptr, ::PROTOBUF_NAMESPACE_ID::internal::ParseContext* ctx) final;
  ::PROTOBUF_NAMESPACE_ID::uint8* _InternalSerialize(
      ::PROTOBUF_NAMESPACE_ID::uint8* target, ::PROTOBUF_NAMESPACE_ID::io::EpsCopyOutputStream* stream) const final;
  int GetCachedSize() const final { return _cached_size_.Get(); }

  private:
  void SharedCtor();
  void SharedDtor();
  void SetCachedSize(int size) const final;
  void InternalSwap(UpdateFirmwareRequest* other);
  friend class ::PROTOBUF_NAMESPACE_ID::internal::AnyMetadata;
  static ::PROTOBUF_NAMESPACE_ID::StringPiece FullMessageName() {
    return "onvif_firmware_update.UpdateFirmwareRequest";
  }
  protected:
  explicit UpdateFirmwareRequest(::PROTOBUF_NAMESPACE_ID::Arena* arena,
                       bool is_message_owned = false);
  private:
  static void ArenaDtor(void* object);
  inline void RegisterArenaDtor(::PROTOBUF_NAMESPACE_ID::Arena* arena);
  public:

  static const ClassData _class_data_;
  const ::PROTOBUF_NAMESPACE_ID::Message::ClassData*GetClassData() const final;

  ::PROTOBUF_NAMESPACE_ID::Metadata GetMetadata() const final;

  // nested types ----------------------------------------------------

  // accessors -------------------------------------------------------

  enum : int {
    kVersionFieldNumber = 1,
    kRebootTimeSecsFieldNumber = 2,
  };
  // string version = 1;
  void clear_version();
  const std::string& version() const;
  template <typename ArgT0 = const std::string&, typename... ArgT>
  void set_version(ArgT0&& arg0, ArgT... args);
  std::string* mutable_version();
  PROTOBUF_MUST_USE_RESULT std::string* release_version();
  void set_allocated_version(std::string* version);
  private:
  const std::string& _internal_version() const;
  inline PROTOBUF_ALWAYS_INLINE void _internal_set_version(const std::string& value);
  std::string* _internal_mutable_version();
  public:

  // optional int32 reboot_time_secs = 2;
  bool has_reboot_time_secs() const;
  private:
  bool _internal_has_reboot_time_secs() const;
  public:
  void clear_reboot_time_secs();
  ::PROTOBUF_NAMESPACE_ID::int32 reboot_time_secs() const;
  void set_reboot_time_secs(::PROTOBUF_NAMESPACE_ID::int32 value);
  private:
  ::PROTOBUF_NAMESPACE_ID::int32 _internal_reboot_time_secs() const;
  void _internal_set_reboot_time_secs(::PROTOBUF_NAMESPACE_ID::int32 value);
  public:

  // @@protoc_insertion_point(class_scope:onvif_firmware_update.UpdateFirmwareRequest)
 private:
  class _Internal;

  template <typename T> friend class ::PROTOBUF_NAMESPACE_ID::Arena::InternalHelper;
  typedef void InternalArenaConstructable_;
  typedef void DestructorSkippable_;
  ::PROTOBUF_NAMESPACE_ID::internal::HasBits<1> _has_bits_;
  mutable ::PROTOBUF_NAMESPACE_ID::internal::CachedSize _cached_size_;
  ::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr version_;
  ::PROTOBUF_NAMESPACE_ID::int32 reboot_time_secs_;
  friend struct ::TableStruct_onvif_5ffirmware_5fupdate_2eproto;
};
// -------------------------------------------------------------------

class UpdateFirmwareReply final :
    public ::PROTOBUF_NAMESPACE_ID::Message /* @@protoc_insertion_point(class_definition:onvif_firmware_update.UpdateFirmwareReply) */ {
 public:
  inline UpdateFirmwareReply() : UpdateFirmwareReply(nullptr) {}
  ~UpdateFirmwareReply() override;
  explicit constexpr UpdateFirmwareReply(::PROTOBUF_NAMESPACE_ID::internal::ConstantInitialized);

  UpdateFirmwareReply(const UpdateFirmwareReply& from);
  UpdateFirmwareReply(UpdateFirmwareReply&& from) noexcept
    : UpdateFirmwareReply() {
    *this = ::std::move(from);
  }

  inline UpdateFirmwareReply& operator=(const UpdateFirmwareReply& from) {
    CopyFrom(from);
    return *this;
  }
  inline UpdateFirmwareReply& operator=(UpdateFirmwareReply&& from) noexcept {
    if (this == &from) return *this;
    if (GetOwningArena() == from.GetOwningArena()) {
      InternalSwap(&from);
    } else {
      CopyFrom(from);
    }
    return *this;
  }

  static const ::PROTOBUF_NAMESPACE_ID::Descriptor* descriptor() {
    return GetDescriptor();
  }
  static const ::PROTOBUF_NAMESPACE_ID::Descriptor* GetDescriptor() {
    return default_instance().GetMetadata().descriptor;
  }
  static const ::PROTOBUF_NAMESPACE_ID::Reflection* GetReflection() {
    return default_instance().GetMetadata().reflection;
  }
  static const UpdateFirmwareReply& default_instance() {
    return *internal_default_instance();
  }
  static inline const UpdateFirmwareReply* internal_default_instance() {
    return reinterpret_cast<const UpdateFirmwareReply*>(
               &_UpdateFirmwareReply_default_instance_);
  }
  static constexpr int kIndexInFileMessages =
    1;

  friend void swap(UpdateFirmwareReply& a, UpdateFirmwareReply& b) {
    a.Swap(&b);
  }
  inline void Swap(UpdateFirmwareReply* other) {
    if (other == this) return;
    if (GetOwningArena() == other->GetOwningArena()) {
      InternalSwap(other);
    } else {
      ::PROTOBUF_NAMESPACE_ID::internal::GenericSwap(this, other);
    }
  }
  void UnsafeArenaSwap(UpdateFirmwareReply* other) {
    if (other == this) return;
    GOOGLE_DCHECK(GetOwningArena() == other->GetOwningArena());
    InternalSwap(other);
  }

  // implements Message ----------------------------------------------

  inline UpdateFirmwareReply* New() const final {
    return new UpdateFirmwareReply();
  }

  UpdateFirmwareReply* New(::PROTOBUF_NAMESPACE_ID::Arena* arena) const final {
    return CreateMaybeMessage<UpdateFirmwareReply>(arena);
  }
  using ::PROTOBUF_NAMESPACE_ID::Message::CopyFrom;
  void CopyFrom(const UpdateFirmwareReply& from);
  using ::PROTOBUF_NAMESPACE_ID::Message::MergeFrom;
  void MergeFrom(const UpdateFirmwareReply& from);
  private:
  static void MergeImpl(::PROTOBUF_NAMESPACE_ID::Message*to, const ::PROTOBUF_NAMESPACE_ID::Message&from);
  public:
  PROTOBUF_ATTRIBUTE_REINITIALIZES void Clear() final;
  bool IsInitialized() const final;

  size_t ByteSizeLong() const final;
  const char* _InternalParse(const char* ptr, ::PROTOBUF_NAMESPACE_ID::internal::ParseContext* ctx) final;
  ::PROTOBUF_NAMESPACE_ID::uint8* _InternalSerialize(
      ::PROTOBUF_NAMESPACE_ID::uint8* target, ::PROTOBUF_NAMESPACE_ID::io::EpsCopyOutputStream* stream) const final;
  int GetCachedSize() const final { return _cached_size_.Get(); }

  private:
  void SharedCtor();
  void SharedDtor();
  void SetCachedSize(int size) const final;
  void InternalSwap(UpdateFirmwareReply* other);
  friend class ::PROTOBUF_NAMESPACE_ID::internal::AnyMetadata;
  static ::PROTOBUF_NAMESPACE_ID::StringPiece FullMessageName() {
    return "onvif_firmware_update.UpdateFirmwareReply";
  }
  protected:
  explicit UpdateFirmwareReply(::PROTOBUF_NAMESPACE_ID::Arena* arena,
                       bool is_message_owned = false);
  private:
  static void ArenaDtor(void* object);
  inline void RegisterArenaDtor(::PROTOBUF_NAMESPACE_ID::Arena* arena);
  public:

  static const ClassData _class_data_;
  const ::PROTOBUF_NAMESPACE_ID::Message::ClassData*GetClassData() const final;

  ::PROTOBUF_NAMESPACE_ID::Metadata GetMetadata() const final;

  // nested types ----------------------------------------------------

  // accessors -------------------------------------------------------

  enum : int {
    kVersionFieldNumber = 1,
  };
  // string version = 1;
  void clear_version();
  const std::string& version() const;
  template <typename ArgT0 = const std::string&, typename... ArgT>
  void set_version(ArgT0&& arg0, ArgT... args);
  std::string* mutable_version();
  PROTOBUF_MUST_USE_RESULT std::string* release_version();
  void set_allocated_version(std::string* version);
  private:
  const std::string& _internal_version() const;
  inline PROTOBUF_ALWAYS_INLINE void _internal_set_version(const std::string& value);
  std::string* _internal_mutable_version();
  public:

  // @@protoc_insertion_point(class_scope:onvif_firmware_update.UpdateFirmwareReply)
 private:
  class _Internal;

  template <typename T> friend class ::PROTOBUF_NAMESPACE_ID::Arena::InternalHelper;
  typedef void InternalArenaConstructable_;
  typedef void DestructorSkippable_;
  ::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr version_;
  mutable ::PROTOBUF_NAMESPACE_ID::internal::CachedSize _cached_size_;
  friend struct ::TableStruct_onvif_5ffirmware_5fupdate_2eproto;
};
// ===================================================================


// ===================================================================

#ifdef __GNUC__
  #pragma GCC diagnostic push
  #pragma GCC diagnostic ignored "-Wstrict-aliasing"
#endif  // __GNUC__
// UpdateFirmwareRequest

// string version = 1;
inline void UpdateFirmwareRequest::clear_version() {
  version_.ClearToEmpty();
}
inline const std::string& UpdateFirmwareRequest::version() const {
  // @@protoc_insertion_point(field_get:onvif_firmware_update.UpdateFirmwareRequest.version)
  return _internal_version();
}
template <typename ArgT0, typename... ArgT>
inline PROTOBUF_ALWAYS_INLINE
void UpdateFirmwareRequest::set_version(ArgT0&& arg0, ArgT... args) {
 
 version_.Set(::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr::EmptyDefault{}, static_cast<ArgT0 &&>(arg0), args..., GetArenaForAllocation());
  // @@protoc_insertion_point(field_set:onvif_firmware_update.UpdateFirmwareRequest.version)
}
inline std::string* UpdateFirmwareRequest::mutable_version() {
  std::string* _s = _internal_mutable_version();
  // @@protoc_insertion_point(field_mutable:onvif_firmware_update.UpdateFirmwareRequest.version)
  return _s;
}
inline const std::string& UpdateFirmwareRequest::_internal_version() const {
  return version_.Get();
}
inline void UpdateFirmwareRequest::_internal_set_version(const std::string& value) {
  
  version_.Set(::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr::EmptyDefault{}, value, GetArenaForAllocation());
}
inline std::string* UpdateFirmwareRequest::_internal_mutable_version() {
  
  return version_.Mutable(::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr::EmptyDefault{}, GetArenaForAllocation());
}
inline std::string* UpdateFirmwareRequest::release_version() {
  // @@protoc_insertion_point(field_release:onvif_firmware_update.UpdateFirmwareRequest.version)
  return version_.Release(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArenaForAllocation());
}
inline void UpdateFirmwareRequest::set_allocated_version(std::string* version) {
  if (version != nullptr) {
    
  } else {
    
  }
  version_.SetAllocated(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), version,
      GetArenaForAllocation());
  // @@protoc_insertion_point(field_set_allocated:onvif_firmware_update.UpdateFirmwareRequest.version)
}

// optional int32 reboot_time_secs = 2;
inline bool UpdateFirmwareRequest::_internal_has_reboot_time_secs() const {
  bool value = (_has_bits_[0] & 0x00000001u) != 0;
  return value;
}
inline bool UpdateFirmwareRequest::has_reboot_time_secs() const {
  return _internal_has_reboot_time_secs();
}
inline void UpdateFirmwareRequest::clear_reboot_time_secs() {
  reboot_time_secs_ = 0;
  _has_bits_[0] &= ~0x00000001u;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 UpdateFirmwareRequest::_internal_reboot_time_secs() const {
  return reboot_time_secs_;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 UpdateFirmwareRequest::reboot_time_secs() const {
  // @@protoc_insertion_point(field_get:onvif_firmware_update.UpdateFirmwareRequest.reboot_time_secs)
  return _internal_reboot_time_secs();
}
inline void UpdateFirmwareRequest::_internal_set_reboot_time_secs(::PROTOBUF_NAMESPACE_ID::int32 value) {
  _has_bits_[0] |= 0x00000001u;
  reboot_time_secs_ = value;
}
inline void UpdateFirmwareRequest::set_reboot_time_secs(::PROTOBUF_NAMESPACE_ID::int32 value) {
  _internal_set_reboot_time_secs(value);
  // @@protoc_insertion_point(field_set:onvif_firmware_update.UpdateFirmwareRequest.reboot_time_secs)
}

// -------------------------------------------------------------------

// UpdateFirmwareReply

// string version = 1;
inline void UpdateFirmwareReply::clear_version() {
  version_.ClearToEmpty();
}
inline const std::string& UpdateFirmwareReply::version() const {
  // @@protoc_insertion_point(field_get:onvif_firmware_update.UpdateFirmwareReply.version)
  return _internal_version();
}
template <typename ArgT0, typename... ArgT>
inline PROTOBUF_ALWAYS_INLINE
void UpdateFirmwareReply::set_version(ArgT0&& arg0, ArgT... args) {
 
 version_.Set(::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr::EmptyDefault{}, static_cast<ArgT0 &&>(arg0), args..., GetArenaForAllocation());
  // @@protoc_insertion_point(field_set:onvif_firmware_update.UpdateFirmwareReply.version)
}
inline std::string* UpdateFirmwareReply::mutable_version() {
  std::string* _s = _internal_mutable_version();
  // @@protoc_insertion_point(field_mutable:onvif_firmware_update.UpdateFirmwareReply.version)
  return _s;
}
inline const std::string& UpdateFirmwareReply::_internal_version() const {
  return version_.Get();
}
inline void UpdateFirmwareReply::_internal_set_version(const std::string& value) {
  
  version_.Set(::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr::EmptyDefault{}, value, GetArenaForAllocation());
}
inline std::string* UpdateFirmwareReply::_internal_mutable_version() {
  
  return version_.Mutable(::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr::EmptyDefault{}, GetArenaForAllocation());
}
inline std::string* UpdateFirmwareReply::release_version() {
  // @@protoc_insertion_point(field_release:onvif_firmware_update.UpdateFirmwareReply.version)
  return version_.Release(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArenaForAllocation());
}
inline void UpdateFirmwareReply::set_allocated_version(std::string* version) {
  if (version != nullptr) {
    
  } else {
    
  }
  version_.SetAllocated(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), version,
      GetArenaForAllocation());
  // @@protoc_insertion_point(field_set_allocated:onvif_firmware_update.UpdateFirmwareReply.version)
}

#ifdef __GNUC__
  #pragma GCC diagnostic pop
#endif  // __GNUC__
// -------------------------------------------------------------------


// @@protoc_insertion_point(namespace_scope)

}  // namespace onvif_firmware_update

// @@protoc_insertion_point(global_scope)

#include <google/protobuf/port_undef.inc>
#endif  // GOOGLE_PROTOBUF_INCLUDED_GOOGLE_PROTOBUF_INCLUDED_onvif_5ffirmware_5fupdate_2eproto
